...
def test_multi_gradient_echo(system_defaults):
    ...
    adc = pp.make_adc(num_samples=adc_total_samples, dwell=adc_dwell_time, system=system, delay=system.adc_dead_time)
    ...
    for idx in range(len(gx_readout_list)):
        seq.add_block(gx_readout_list[idx], gy_readout_list[idx])
        seq.add_block(pp.make_delay(30e-3))
