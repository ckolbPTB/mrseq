...
class DiffusionPrep:
    ...
    def __init__(
        ...
        self._time_between_diff_and_rf = round_to_raster(
            (g_delta_time - min_g_delta_time) / 2, raster_time=self._system.block_duration_raster
        )
        self._time_between_rf_and_diff = round_to_raster(
            (g_delta_time - min_g_delta_time) - self._time_between_diff_and_rf,
            raster_time=self._system.block_duration_raster,
        )
        ...
        self._delay_after_crush = round_to_raster(
            pp.calc_duration(self._g_diff) - pp.calc_duration(self._g_crush) + self._time_between_rf_and_diff,
            raster_time=self._system.block_duration_raster,
        )
    ...
    def add_diffusion_prep(self, seq: pp.Sequence | None, b_value: float):
        ...
        if b_value > 0:
            seq.add_block(pp.make_delay(self._time_between_rf_and_diff))
            seq.add_block(g_current_diff)
        else:
            seq.add_block(self._g_crush)
