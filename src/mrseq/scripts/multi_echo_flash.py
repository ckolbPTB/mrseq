"""2D Cartesian multi-echo FLASH sequence."""
...
def multi_echo_flash_kernel(
    ...
    min_delta_te = pp.calc_duration(gx) + pp.calc_duration(gx_between)
    if delta_te is None:
        delta_te_delay = 0.0
    else:
        delta_te_delay = round_to_raster(delta_te - min_delta_te, system.block_duration_raster)
        if delta_te_delay < 0:
            raise ValueError(
    ...
    min_tr = (
        ...
        + gzr_gx_dur  # slice selection re-phasing gradient and readout pre-winder
        + pp.calc_duration(gx) * n_echoes  # readout gradients
        + pp.calc_duration(gx_between) * (n_echoes - 1)
        + pp.calc_duration(gx_post)  # readout-re-winder
        + pp.calc_duration(gz_spoil)  # gradient spoiler
    )
    ...
    for rep in range(n_repetitions):
        ...
        for pe in range(-n_dummy, n_phase_encoding):
            ...
            for echo in range(n_echoes):
                ...
                if echo < n_echoes - 1:
                    seq.add_block(pp.make_delay(delta_te_delay))
                    seq.add_block(gx_between)
...
def main(
    ...
    """Generate a 2D multi-echo Cartesian FLASH sequence.

    This sequence uses a monopolar readout. For a bipolar readout see t2star_multi_echo_flash.py.
    ...
    filename = f'{Path(__file__).stem}_{int(fov_xy * 1000)}fov_xy_{int(slice_thickness * 1000)}_slice_thickness_'
    filename += f'{n_readout}nx_{n_phase_encoding}ny'
