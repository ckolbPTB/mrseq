"""Cartesian turbo-spin echo sequence for B0 mapping."""
...
def b0_tse_kernel(
    ...
    """Generate a Cartesian TSE sequence for B0-mapping.
    ...
    delta_te
        Echo-time shifts for B0 mapping.
    n_echoes
    ...
    min_te = 2 * max(
        ...
        round_to_raster(min_tau3 + max(delta_te), system.block_duration_raster),
    )
    ...
    for dte_idx, dte in enumerate(delta_te):
        # set contrast ('ECO') label for current inversion time
        contrast_label = pp.make_label(type='SET', label='ECO', value=int(dte_idx))
        for se in range(n_slice_encoding):
            ...
            for pe in range(n_phase_encoding):
                ...
                for echo in range(n_echoes):
                    ...
                    seq.add_block(pp.make_delay(tau2 + dte))
                    ...
                    if echo < n_echoes - 1:
                        seq.add_block(pp.make_delay(tau3 - dte))
...
def main(
    ...
    """Generate Cartesian TSE sequence for B0-mapping.
    ...
    te
        Desired echo time (TE) (in seconds). Minimum echo time is used if set to None.
    delta_te
        Echo-time shifts for B0 mapping. If set to None, default values of [0, 0.0004]s are used.
    n_echoes
        Number of echoes in one TSE train.
    tr
        Desired repetition time (TR) (in seconds).
